...
influx setup --skip-verify --bucket ${DOCKER_INFLUXDB_INIT_BUCKET} --org ${DOCKER_INFLUXDB_ORGANIZATION} --retention ${DOCKER_INFLUXDB_INIT_RETENTION} --token ${DOCKER_INFLUXDB_INIT_ADMIN_TOKEN} --username ${DOCKER_INFLUXDB_INIT_USERNAME} --password ${DOCKER_INFLUXDB_INIT_PASSWORD} --host http://${DOCKER_INFLUXDB_INIT_HOST}:8086 --force
influx bucket create --name servers --org ${DOCKER_INFLUXDB_ORGANIZATION} --retention 365d --token ${DOCKER_INFLUXDB_INIT_PASSWORD}